...
const delayout = (data) => {
  ...
  const chunks = data.chunks.map((chunk) => copyExcluding(chunk, [
    "address"
  ]));
...
const layout = (data) => {
  ...
  const chunks = data.chunks.map((chunk) => {
    ...
    offset += BigInt(chunk.data.length);

    return result;
  });

  const resolveOffset = (s) => {
    if (s.chunk_idx !== undefined) {
      const chunk = chunks[s.chunk_idx];
      assert(!!chunk, "referenced chunk not available");
      return chunk.address + s.chunk_offset;
    } else {
      return BigInt(0);
    }
  };
