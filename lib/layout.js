...
const layout = (data) => {
  ...
  const segments = data.segments.map((segment) => {
    return Object.assign({}, segment, {
      "p_offset": segment.junk_idx !== undefined ? junks[segment.junk_idx].address + segment.junk_offset : BigInt(0)
    });
  });

  const sections = data.sections.map((section) => {
    return Object.assign({}, section, {
      "sh_offset": section.junk_idx !== undefined ? junks[section.junk_idx].address + section.junk_offset : BigInt(0)
    });
  });
