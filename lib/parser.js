    ...
    const format = (data) => {
      ...
      Object.keys(defs).forEach((field) => {
        ...
        if (data[field] === undefined) {
          throw Error(`field ${field} is undefined`);
        }
...
const splitEntries = ({ data, entrySize }, fn) => {
  ...
  assert(BigInt(data.length) % entrySize === BigInt(0), "buffer size must be multiple of entry size, size = " + data.length + " entrySize = " + entrySize);
