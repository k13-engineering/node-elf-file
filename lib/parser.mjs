import EndianBuffer from "./buffer.mjs";
...
const structure = {
  "create": (defs, { endianness }) => {
    const read = (data, field) => {
      ...
      return def.codec ? def.codec.decode(value, BigInt(value)) : BigInt(value);
    };
    ...
    const format = (data) => {
      ...
      Object.keys(defs).forEach((field) => {
        ...
        let value = def.codec && typeof data[field] === "string" ? def.codec.encode(data[field]) : data[field];
...
const splitEntries = ({ data, entrySize }, fn) => {
  ...
  assert.equal(BigInt(data.length) % entrySize, 0n);

  return Array.from({ "length": Number(BigInt(data.length) / entrySize) }, (_, idx) => {
    return data.slice(Number(BigInt(idx) * entrySize), Number((BigInt(idx) + 1n) * entrySize));
  });
};
